using System.Text;
using API.Infrastructure.Common.Caching;
//using FCB.Application.Common.Interfaces;
...
public class DistributedCacheService : ICacheService
{
    ...
    private readonly ILogger<DistributedCacheService> _settingsger;
    ...
    public DistributedCacheService(IDistributedCache cache, ISerializerService serializer, ILogger<DistributedCacheService> logger) =>
        (_cache, _serializer, _settingsger) = (cache, serializer, logger);
    ...
    public async Task RefreshAsync(string key, CancellationToken token = default)
    {
        try
        {
            ...
            _settingsger.LogDebug(string.Format("Cache Refreshed : {0}", key));
        }
        catch
        {
        }
    ...
    private void Set(string key, byte[] value, TimeSpan? slidingExpiration = null)
    {
        try
        {
            ...
            _settingsger.LogDebug($"Added to Cache : {key}");
        }
        catch
        {
        }
