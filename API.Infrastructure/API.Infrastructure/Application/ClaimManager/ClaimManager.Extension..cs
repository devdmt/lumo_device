...
namespace API.Infrastructure.Application.ClaimManager
{
    ...
    {
        public async Task<(string,string)> UploadCreditLifeFileAsync(FileDetails request, string customername)
        {
            ...
            if (streamData.Length > 0)
            {
                ...
                if (request.extension.ToLower().Contains("xls") || request.extension.ToLower().Contains("xlsx"))
                {
                    folderName = Path.Combine("CreditLife", folder);
                    fileName = fileName.ReplaceWhitespace("-") + "_" + DateTime.Now.ToString("ddMMyyHHmmss");
                }
                ...
                fileName += "." + request.extension.Trim().Replace(".", "");
                ...
                return (fileName, fullPath);// dbPath.Replace("\\", "/");
            }
            else
            {
         ...
         public async Task<(string, Byte[])> UploadFileBytesAsync(FileDetails request, string customername)
        {
           

            string FileType = ".jpg,.png,.jpeg,.pdf,.docx,.xls,.xlsx";
            ...
            if (streamData.Length > 0)
            {
                ...
                string folderName = Path.Combine("Files", "Others", folder);
                string fileName = request.name.Trim('"');
                fileName = RemoveSpecialCharacters(fileName.Replace(request.extension.Trim(),""));
                ...
                return  (dbPath,  Convert.FromBase64String(base64Data));
            }
            else
            {
                return (string.Empty, []);
